    ...
    <main className="dashboard-shell">
      ...
      {tab === "logistics" && <>
        ...
        <section className="analytics-grid logistics-grid">
          ...
          <ChartPanel kicker="PRIORIZAÇÃO" title="Sellers para intervenção" className="full table-panel">
            <div className="table-wrap"><table><thead><tr><th>Seller</th><th>UF</th><th>GMV</th><th>Pedidos</th><th>SLA</th><th>Atraso médio</th><th>Classificação</th></tr></thead><tbody>{[...sellers].sort((a, b) => a.sla - b.sla).map((item) => <tr key={item.id}><td><code>{item.id}</code></td><td>{item.state}</td><td>{currency.format(item.gmv)}</td><td>{integer.format(item.orders)}</td><td>{percent.format(item.sla)}</td><td>{decimal.format(item.avgDelay)} d</td><td><span className={`badge ${item.sla < .86 ? "critical" : item.sla < .9 ? "warning" : "healthy"}`}>{item.sla < .86 ? "Crítico" : item.sla < .9 ? "Atenção" : "Saudável"}</span></td></tr>)}</tbody></table></div>
          </ChartPanel>
        </section>
      </>}
      ...
      {tab === "quality" && <section className="analytics-grid quality-grid">
        ...
        <ChartPanel kicker="CONTRATOS" title="Resultado dos testes" className="quality-table table-panel">
          <div className="table-wrap"><table><thead><tr><th>Teste</th><th>Escopo</th><th>Cobertura</th><th>Falhas</th><th>Status</th></tr></thead><tbody>{data.quality.map((item) => <tr key={item.name}><td>{item.name}</td><td>{item.scope}</td><td>{percent.format(item.coverage)}</td><td>{integer.format(item.failedRows)}</td><td><span className={`badge ${item.status === "passed" ? "healthy" : "warning"}`}>{item.status === "passed" ? "Aprovado" : "Alerta"}</span></td></tr>)}</tbody></table></div>
        </ChartPanel>
